...
When the ask-questions tool is available, approval should be collected inline via `vscode/askQuestions` instead of relying on a follow-up freeform chat message.
...
6. If the `vscode/askQuestions` tool is available, use it immediately to request explicit approval in the current conversation flow.
...
- Do not prefer a plain trailing question over `vscode/askQuestions` when that tool is available.
...
When `vscode/askQuestions` is available, the approval prompt should use inline choices so the user can confirm from the chat UI without composing a separate follow-up request.
